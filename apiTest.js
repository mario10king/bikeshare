...
function controller(origin, destination) {
  return getStationsInfo.then(function(info) {
    ...
    return googleAPI(origin, destination, formattedStations).then(function(
      ids
    ) {
...
var getStationsInfo = fetch(
  'http://santamonicabikeshare.com/opendata/station_information.json'
)
